...
import { Dimmer, Divider, DropdownProps, Form, InputOnChangeData, Loader, TextAreaProps } from 'semantic-ui-react';
...
export class EditProjectPage extends React.Component<IEditProjectPageProps> {
  ...
  public createEdited() {
    if (this.props.match.params.id) {
      this.mode = Mode.Edit;
      this.edited = this.props.listProject.find((p) => p.id === +this.props.match.params.id);
      if (this.edited) {
  ...
  public render(): JSX.Element {
    ...
    const itemUser = listUser.filter((u) => u.teamId === this.edited.teamId).map((v) => ({
    ...
    const itemBranch = listBranch.filter((b) => b.agencyId === this.edited.agencyId).map((v) => ({
      text: v.name,
      value: v.id,
    }));
    const itemTeam = listTeam.filter((t) => t.branchId === this.edited.branchId).map((v) => ({
      text: v.name,
      value: v.id,
    }));
    ...
    const itemProject = listProject.filter((p) => p.clientId === this.edited.clientId).map((v) => ({
      text: v.name,
      value: v.id,
    }));
    ...
    return (
      ...
      >
        ...
        <Divider />
        <TextArea
          onChange={(e: React.FormEvent, { value }: TextAreaProps) => this.onChangeProperty('comment')(value)}
          label="Comment"
          enabled={this.enabled()}
